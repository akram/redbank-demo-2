#!/bin/bash
#
# Register the orchestrator's public URL as a valid OAuth redirect URI
# in the Keycloak client. Run this after deploying the orchestrator so
# the route already exists.
#
# Idempotent — safe to run multiple times.  Merges the orchestrator URL
...
#   NAMESPACE           Namespace where the orchestrator is deployed (default: redbank-demo)
#   ORCHESTRATOR_URL    Orchestrator public URL (auto-detected from oc route)
...
if [[ -z "${ORCHESTRATOR_URL:-}" ]]; then
  _orch_host=$(oc get route redbank-orchestrator -n "${NAMESPACE}" -o jsonpath='{.spec.host}' 2>/dev/null) || true
  if [[ -n "${_orch_host}" ]]; then
    ORCHESTRATOR_URL="https://${_orch_host}"
  else
    echo "ERROR: Cannot detect orchestrator URL. Is the orchestrator deployed in namespace '${NAMESPACE}'?" >&2
    echo "  Set ORCHESTRATOR_URL explicitly or deploy the orchestrator first." >&2
    exit 1
  fi
fi
_out "Orchestrator URL: ${ORCHESTRATOR_URL}"
...
NEW_REDIRECT="${ORCHESTRATOR_URL}/*"
NEW_ORIGIN="${ORCHESTRATOR_URL}"
...
if [[ "$CURRENT_LOGOUT" != *"${ORCHESTRATOR_URL}"* ]]; then
